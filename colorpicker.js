	...
	var ColorPicker = function () {
        ...
        onCancel: function () {},
			...
			change = function (ev) {
				...
				cal.data('colorpicker').onChange.apply(cal, [col, HSBToHex(col), HSBToRGB(col),  cal.data('colorpicker').parent, cal.data('colorpicker').el ]);
			},
			blur = function (ev) {
				var cal = $(this).parent().parent();
        ...
        if (ev.data !== undefined) {
            var cal = ev.data.cal; // from "$(document).on('mousedown' ..."
        } else {
            var cal = $(this).parent();
        }
        var col = cal.data('colorpicker').color;
        setNewColor(cal.data('colorpicker').origColor, cal.get(0)); // set current color to original color
        var el = cal.data('colorpicker').el;
        $(el).ColorPickerHide();
        cal.data('colorpicker').onCancel.apply(cal, [HSBToHex(cal.data('colorpicker').origColor), cal.data('colorpicker').el ]);
			},
			show = function (ev) {
				...
				$(document).on('mousedown', {cal: cal}, hide); // TODO: change to be "cancel"
				return false;
			},
			hide = function (ev) {
		...
		return {
            ...
            cal.find('div.colorpicker_cancel')
              .on('mouseenter', enterCancel)
							.on('mouseleave', leaveCancel)
